...
/opt/mssql-tools18/bin/sqlcmd -S localhost -U sa -P 'YourStrong!Passw0rd' -C -Q "
IF NOT EXISTS (SELECT * FROM sys.databases WHERE name = N'translator-message-db-test')
  CREATE DATABASE [translator-message-db-test];
"

/opt/mssql-tools18/bin/sqlcmd -S localhost -U sa -P 'YourStrong!Passw0rd' -C -d translator-message-db-test -Q "
